...
//							controller.bindActionPopup();
	...
	handleSaveSuccess : function(data){
		...
		/*function(oAction){ R1.5 for navigation to WCM
			//if(controller.navToWCM)// If request to redirect
			//	controller.confirmedNavToWCM(controller.navData);
		} */
		if(data){
			...
			if(message)
			{
				message = message[0];
				if(message.TYPE === "S")
					sap.m.MessageBox.show(successMsg,sap.m.MessageBox.Icon.SUCCESS,"Save Success!", sap.m.MessageBox.Action.OK, function(oAction){
						if(controller.navToWCM){
							controller.confirmedNavToWCM(controller.navData);
						}
					});
...
//			icon : "sap-icon://functional-location",
	...
	showPerformance : function(evt){
		...
		var inspType = "";
		var shiftName = "";
		$.each(selectedListOption, function(ind, item){
			if($.trim(orders).length > 0)
				orders = orders + ",";
			orders = orders + item.Aufnr;
			setNo = item.SetNo;
			inspType = item.InspType;
			shiftName = item.ShiftName;
		});

		var taskId = taskData.TaskCode;	
		this.navData = {
				"orders": orders,
				"setNo" : setNo,
				"taskId" : taskId,
		...
		// Determine the navigation based on the below flow
		//The ZPMT_WRKCONFIRM custom table is extended with an additional field Wheel Measurement ZZWHEEL_MEAS.  
		//If this field is selected along with measurement exist ZZMEAS_EXIST in ZPMT_WRKCONFIRM then pop-up a confirmation message 
		//to save the work done
		if($.trim(taskData.MeasPoints) === 'X' && $.trim(taskData.WheelMeas) === 'X'){
	...
	confirmedNavToWCM: function(data){
		// Below code navigate between cross applications --  here it is to fault management
/*		if (sap.ushell && sap.ushell.Container && sap.ushell.Container.getService)
		{
			var oCrossAppNavigator = sap.ushell.Container.getService("CrossApplicationNavigation");
			oCrossAppNavigator.toExternal({
				target: { semanticObject : "WHEEL_CONDITION_MEASUREMENT", action: "open" },   //the app you're navigating to 
				params : { "from": "TMP", "orders": data.orders, "set":data.setNo, "taskId":data.taskId,"inspType":data.inspType, "shiftName": data.shiftName, "selectedKey": data.selectedKey}
			});
		}
		else
		{
...
//        vRouter.navTo("Create",vController.mfsRouteData,false);
        vRouter.navTo("jMasterWOList",{orders:data.orders, fromView:"jobCard"},true);
...
//		this.bindActionPopup();
	...
	showDetailTask: function(evt){
		...
		var cars = headerData.CarNo.split(",");
            ...
            if(cars.length > 0){ // Navigate if task has more than one car
                  //NARASIMB Defect 10066 allow for any cars
