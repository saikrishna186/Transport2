...
//open document
listPress : function(evt) {var docValue = evt.getParameters().listItem.data("documentId");
//    this.router.navTo("DocView",{docValue:docValue});
			var faultServiceUrl = applicationContext.applicationEndpointURL + "/ZGWP_PM_FAULT_MANAGE_SRV";
			var pdf_url = faultServiceUrl
					+ "/ETS_GET_ATTACH_CONTENTS(SoObjNo='"
					+ docValue + "')/$value";
                                 sap.m.URLHelper.redirect(pdf_url,true);
...
uploadDialog: function(){
	  ...
	  var eamFaultServiceURL = applicationContext.applicationEndpointURL + "/ZGWP_PM_FAULT_MANAGE_SRV"; //"/sap/opu/odata/sap/ZGWP_PM_FAULT_MANAGE_SRV";
  var notifNum = this.getOwnerComponent().notifnum;
	var controller = this;
		...
		var oFU = new sap.ui.unified.FileUploader({
				...
				uploadUrl : eamFaultServiceURL+"/ETS_CREATE_GOS_INPUT",  
name: 'FileUpload',
buttonText:'Browse',
width: "89%",
