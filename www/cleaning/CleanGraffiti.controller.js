sap.ui.define([ "sap/ui/core/mvc/Controller" ], function(BaseController) {
	...
	return BaseController.extend("tfnsw.eam.cleaning.view.CleanGraffiti", {
		...
		onInit : function() {
			...
			var oTable_create = new sap.m.Table("clean_create", {
			...
			this.getView().byId("id_InternalTotal").attachBrowserEvent("keypress", function(event){
			...
			this.getView().byId("id_InternalCleaned").attachBrowserEvent("keypress", function(event){
				 if (String.fromCharCode(event.keyCode).match(/[^0-9\.]/g)) 
				 {
					 event.preventDefault()
				 }
			...
			this.getView().byId("id_ExtAboveTotal").attachBrowserEvent("keypress", function(event){
				 if (String.fromCharCode(event.keyCode).match(/[^0-9\.]/g)) 
				 {
					 event.preventDefault()
				 }
			...
			this.getView().byId("id_ExtAboveCleaned").attachBrowserEvent("keypress", function(event){
				 if (String.fromCharCode(event.keyCode).match(/[^0-9\.]/g)) 
				 {
					 event.preventDefault()
				 }
			...
			this.getView().byId("id_ExtBelowTotal").attachBrowserEvent("keypress", function(event){
				 if (String.fromCharCode(event.keyCode).match(/[^0-9\.]/g)) 
				 {
					 event.preventDefault()
				 }
			...
			this.getView().byId("id_ExtBelowCleaned").attachBrowserEvent("keypress", function(event){
				 if (String.fromCharCode(event.keyCode).match(/[^0-9\.]/g)) 
				 {
					 event.preventDefault()
				 }
			...
			this.getView().byId("numberofcleanersinput").attachBrowserEvent("keypress", function(event){
				 if (String.fromCharCode(event.keyCode).match(/[^0-9\.]/g)) 
				 {
					 event.preventDefault()
				 }
		...
		validateMatNumber : function(matNumber) {
			...
			if (validMat) {
				...
				//Set the description of Location
				this.getView().byId("cleanLocDesc").setText(validMat[0].Ktext);
			} else {
				matFlag = false;
				//Set the description of Location
				this.getView().byId("cleanLocDesc").setText("");
			}
			return matFlag;
		},
		...
		handleSetChange : function(evt) {
			...
			if (!checkFlag) {
				// #10131 - NARASIMB
				jQuery.sap.require("sap.m.MessageBox");
				sap.m.MessageBox.show("Please provide valid Set", 
						{title 	: "Error", 
						 icon	: sap.m.MessageBox.Icon.ERROR,
						 actions: sap.m.MessageBox.Action.CLOSE,                 
						 onClose: null    ,
						 });
				// End of #10131 - NARASIMB
				evt.getSource().setValueState("Error");
				evt.getSource().setValueStateText(
		...
		setFaultNotification: function(responseData){
			// Defect #10655 - narasimb 17052016
			jQuery.sap.require("sap.m.MessageBox");
			if(responseData){
				var msg = "";
				var msgType = responseData.ZzerrorInd;
				if($.trim(msgType) === "S" || $.trim(msgType) === ""){
					msg = "Fault Notification Created - " + responseData.Zzaufnr;
				...
				else if($.trim(msgType) === "E") {
					msg = responseData.Zzerror;
					sap.m.MessageBox.show(msg, 
							{title 	: "Error", 
							 icon	: sap.m.MessageBox.Icon.ERROR,
							 actions: sap.m.MessageBox.Action.CLOSE,                  // default
				...
				sap.m.MessageBox.show("Error in Submitting the data", 
						{title 	: "Error", 
						 icon	: sap.m.MessageBox.Icon.ERROR,
						 actions: sap.m.MessageBox.Action.CLOSE,                  // default
						 onClose: null    ,
			...
			// End of Defect #10655 - narasimb 17052016
		...
		_onPressSapmbnumberofcleanersdown : function(evt) {
			var cleanerId = this.getView().byId(
					"numberofcleanersinput");
			var cleanerValue = cleanerId.getValue();
			if (cleanerValue > 0 && cleanerValue < 100) {
				cleanerValue = parseInt(cleanerValue) - 1;
				cleanerId.setValue(cleanerValue);
			}
		},
		...
		_onPressInternalTotaldown : function(evt) {
			var intTotalId = this.getView()
					.byId("id_InternalTotal");
			var intTotalValue = intTotalId.getValue();
			if (intTotalValue > 0 && intTotalValue < 1000) {
				intTotalValue = parseInt(intTotalValue) - 1;
				intTotalId.setValue(intTotalValue);
		...
		_onPressExtAbvTotaldown : function(evt) {
			...
			if (extAbvTotalValue > 0 && extAbvTotalValue < 1000) {
		...
		_onPressExtAbvGrafCleaneddown : function(evt) {
			...
			if (extAbvGrafValue > 0 && extAbvGrafValue < 1000) {
		...
		_onPressExtBelTotaldown : function(evt) {
			...
			if (extBelTotalValue > 0 && extBelTotalValue < 1000) {
		...
		_onPressExtBelGrafCleaneddown : function(evt) {
			...
			if (extBelGrafValue > 0 && extBelGrafValue < 1000) {
			...
			if(this.getView().byId("tabBar").getSelectedKey() == component+"---CleanGraffiti--graffitiTab"){
				...
				this.oModel.graffiti.externalBelowGutter.remaining == 0)
					...
					if(this.getView().byId("id_Comments").getValue() == "" &&
					   this.selectedGraffitiTypesId == ""){
						error = "X";
						sap.m.MessageBox.alert("Comments & Graffiti Type are mandatory when Graffiti Remaining > 0", {title : "Error", icon: sap.m.MessageBox.Icon.ERROR});
					};
