	...
	return Controller.extend("tfnswequip.tfnswmfs.controller.Attachment", {
		...
		_handleRouteMatched: function(evt) {
			...
			if(this.sapFlag === true){
				...
				this.getAwsDetails();
				if(this.fullBucket && this.fullBucket !== ""){
					this.getAWSAttachments(this, this.faultId);
				}else{
					sap.m.MessageBox.show(vController.mFSMsg.getProperty("awsDetailsError"), {
		...
		getAWSAttachments:function(vController, faultId) {
			...
			var s3 = new AWS.S3();
			var params = {
					Bucket: vController.fullBucket, /* required */
					Prefix:faultId,
					MaxKeys:100,
					EncodingType:"url"
			};
				...
				success: function(data, response) {
					...
					for (var i = 0; i < data.results.length; i++) {
						if((/\.(bmp|csv|doc|docx|gif|jpeg|jpg|pdf|png|ppt|pptx|tiff|xls|xlsx|txt)$/i).test(data.results[i].Filename)){
							...
							data.results[i].removable = false;
...
//			var imageType = oEvent.getSource().getProperty("imageType");
			...
			var icon = oEvent.getSource().getProperty("Filename"); 
			if(icon !== "IconAddImage.png"){
		...
		uploadDialog: function(vController){
			...
			var oFU = new sap.ui.unified.FileUploader ({
				...
				fileSizeExceed:function(oEvent){
					jQuery.sap.require("sap.m.MessageBox");
					sap.m.MessageBox.show("Maximum of 3MB file is allowed, please try with a lower size / resolution ",sap.m.MessageBox.Icon.ERROR ,"Error" , sap.m.MessageBox.Action.OK, function() { });
					vController.busyIndicator.close();
				},
				          ...
				          new sap.m.Button({
				        	  ...
				        	  press : function(evt) {
				        		  ...
				        		  if(vController.sapFlag){
				        			  ...
				        			  var ext = fileName.substring(fileName.lastIndexOf(".")+1);
				        			  var newFileName = fName+"_"+Date.now()+"."+ext;
				        			  var key=vController.faultId+"/"+newFileName;
				        			  if(f){
					        			  var newtile ={
					        					  Filename : newFileName,
					        					  Qmnum : vController.faultId,
					        					  SoObjNo :  vController.faultId + "/" + newFileName,
					        					  imageType : true,
					        					  imgSrc : vController.url+ vController.faultId+"/"+ newFileName,
					        					  removable : false,
					        					  src : vController.url+ vController.faultId+"/"+ newFileName
					        			  };
					        			  var params2 = {
					        					  Bucket: vController.fullBucket, /* required */
					        					  Key:key,
...
//					        					  models.syncDocument(vController,vController.faultId,vController.mFSMsg);
					        					  modal.busyIndicator.close();
					        					  ...
					        					  sap.m.MessageBox.show(vController.mFSMsg.getProperty("fileLoaded"), {
					        						  ...
					        						  onClose: function(oAction) {
					        							  vController.getAWSAttachments(vController, vController.faultId);
//					        							  vController.getView().getModel("attachments").oData.files.push(newtile);
		...
		getAwsDetails:function(){
			if(this.getOwnerComponent().getModel("awsmodel")){
//				models.getAmazonAWSServerDateString();
				this.uName = this.getOwnerComponent().getModel("awsmodel").oData.Uname;
				this.aKey = this.getOwnerComponent().getModel("awsmodel").oData.AccessKey;
				this.sKey = this.getOwnerComponent().getModel("awsmodel").oData.SecretKey;
				this.fullBucket = this.getOwnerComponent().getModel("awsmodel").oData.FullBucket;
				this.thumbnail = this.getOwnerComponent().getModel("awsmodel").oData.Thumbnail;
				this.url = this.getOwnerComponent().getModel("awsmodel").oData.Url;
				AWS.config.update({
